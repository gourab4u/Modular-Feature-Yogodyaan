// migration-script.js - Complete working version
const fs = require('fs');
...
function generateFeatureIndex(featureName, config) {
  ...
  if (config.contexts && config.contexts.length > 0) {
    ...
    config.contexts.forEach(context => {
      ...
      content += `export { default as ${contextName} } from './contexts/${context}';\n`;
    });
    content += '\n';
  }
...
function migrateToFeatureStructure() {
  ...
  Object.entries(MIGRATION_CONFIG.features).forEach(([featureName, config]) => {
    ...
    // Move components
    config.components.forEach(component => {
      const componentSubfolder = getComponentSubfolder(component);
      const oldPath = path.join(srcPath, 'components', componentSubfolder, component);
...
`;
  ...
  console.log(`Created shared index file: ${path.join(sharedPath, 'index.ts')}`);
  
  // Step 7: Create main features index
...
`;
  ...
  fs.writeFileSync(path.join(srcPath, 'features', 'index.ts'), featuresIndexContent);
  console.log(`Created features index file: ${path.join(srcPath, 'features', 'index.ts')}`);
  ...
  console.log('5. Update your App.tsx with new imports');
}
