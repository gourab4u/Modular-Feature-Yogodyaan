...
export function useClassAssignments(instructorId?: string, status = 'all', dateRange?: { from: Date; to: Date }) {
  ...
  const updateAssignmentStatus = async (assignmentId: string, action: string, data?: any) => {
    try {
  ...
  const markClassCompleted = async (assignmentId: string, remarks?: string) => {
    try {
