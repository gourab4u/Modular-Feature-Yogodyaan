...
import { isValidRating } from '../../../shared/types/ratings';
/**
 ...
 */
export function useClassRating(assignmentId) {
    const [userRating, setUserRating] = useState(null);
    const [aggregate, setAggregate] = useState(null);
    const [loading, setLoading] = useState(true);
    const [submitting, setSubmitting] = useState(false);
    const [error, setError] = useState(null);
    const [eligible, setEligible] = useState(false); // whether user can submit/update (attendance + class ended)
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            if (aggRows && aggRows.length > 0) {
                const sum = aggRows.reduce((acc, r) => acc + (r.rating || 0), 0);
    ...
    const submit = useCallback(async (rating, comment) => {
        ...
        if (!eligible)
            throw new Error('Not eligible to rate this class yet');
        try {
            setSubmitting(true);
            setError(null);
