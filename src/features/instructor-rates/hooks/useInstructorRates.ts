...
// Define a type for the rate data (no longer needs instructor info)
export interface RateWithInstructor extends InstructorRate {}

export const useInstructorRates = (userId: string | undefined) => {
  const [rates, setRates] = useState<RateWithInstructor[]>([]);
  ...
  const addRate = async (rate: Omit<InstructorRate, 'id' | 'created_by'>) => {
    if (!userId) throw new Error("User must be logged in to add a rate");
    try {
      ...
      if (data) {
        ...
        const newRate: RateWithInstructor = { 
          ...data,
  ...
  const updateRate = async (rateId: string, updates: Partial<Omit<InstructorRate, 'id' | 'created_by'>>) => {
    try {
      const { data, error } = await supabase
        .from('instructor_rates')
        .update(updates)
      ...
      if (data) {
        ...
        const updatedRate: RateWithInstructor = { 
          ...data,
          class_types: classType,
          class_packages: classPackage,
        };
