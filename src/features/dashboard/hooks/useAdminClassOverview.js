 ...
 */
export function useAdminClassOverview(filters, opts = {}) {
    const { autoReloadMs = 60000, limit = 1000 } = opts;
    ...
    const load = useCallback(async () => {
        try {
            ...
            // Date filters
            if (filters.from)
                query = query.gte('date', filters.from.toISOString().split('T')[0]);
            if (filters.to)
            ...
            if (filters.search) {
                ...
                list = list.filter(r => r.assignment_id.toLowerCase().includes(s) ||
            ...
            if (list.length > 0) {
                ...
                for (let i = 0; i < ids.length; i += chunkSize) {
                    ...
                    (ratingRows || []).forEach(rr => {
                        if (typeof rr.rating === 'number')
                            ratings.push(rr.rating);
                    });
                }
 ...
 */
export function useAdminAssignmentRoster(assignmentId) {
