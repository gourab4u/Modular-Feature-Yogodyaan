...
export const EditAssignmentModal = ({ assignment, isVisible, bookings, onClose, onSave, onRefresh }) => {
    ...
    useEffect(() => {
        if (assignment) {
            ...
            const paymentType = assignment.payment_type || 'per_class';
    ...
    const calculateCurrentPaymentAmount = (currentStudentCount) => {
        const paymentType = formData.payment_type || assignment.payment_type || 'per_class';
        switch (paymentType) {
        ...
        try {
            ...
            if (bookingsChanged && isSeriesAssignment) {
                ...
                if (classesToUpdate.length === 0) {
                    ...
                    const userConfirmed = confirm(`This will update booking assignments for ${classCount} class${classCount !== 1 ? 'es' : ''} in this series.\n\n` +
