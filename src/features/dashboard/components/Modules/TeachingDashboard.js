...
export default function TeachingDashboard() {
    ...
    const { assignments, loading: assignmentsLoading, error: assignmentsError, refetch: refetchAssignments, updateAssignmentStatus } = useClassAssignments(user?.id); // Fetch all assignments, filter on client side
    ...
    const rejectedCount = assignments?.filter(a => a.instructor_status === 'rejected').length || 0;
    const acceptedCount = assignments?.filter(a => a.instructor_status === 'accepted').length || 0;
    const completedCount = assignments?.filter(a => a.class_status === 'completed').length || 0;
